...
nd <- as.data.frame(lsmeans$emmeans) %>% 
  mutate(trt = factor(trt, levels = c("Controle", "Atectra", "Atectra + Vapor Grip", "Atectra + Roundup Transorb R", "Atectra + Roundup Transorb R + Vapor Grip", "Atectra + Roundup WG", "Atectra + Roundup Original Mais", "Atectra + Roundup Original Mais + Vapor Grip", "Atectra + Roundup Transorb"), labels =c("Controle", "DGA", "DGA + RD", "DGA + Glifosato sal potássico", "DGA + Glifosato sal potássico + RD", "DGA + Glifosato sal de amônio", "DGA + Glifosato sal de di-amônio", "DGA + Glifosato sal de di-amônio + RD", "DGA + Glifosato sal de isopropilamina")))
...
geom_linerange(aes(ymin = lower.CL, ymax = upper.CL), size=1.5) + 
  annotate("text", x = 1, y = 500, label = "P-valor = 0,39", size=5) +
...
geom_linerange(aes(ymin = lower.CL, ymax = upper.CL), size=1.5, color="black") + 
  annotate("text", x = 0.7, y =5500, label = "P-valor = 0,39", size=5) +
theme(axis.title = element_text(size=16), panel.grid = element_blank(),
axis.text = element_text(size=15, color="black"),
legend.position = "none") +
coord_flip() +
...
```{r}
ggplot(fito, aes(y=distance, x=fito)) + #geom_bar(stat="summary", fun.y="mean")
  geom_density_ridges(scale=2) + facet_grid(trt~dat) +
  labs(x="Índice de fitotoxicidade na soja", y="") + 
...
legend.position = "none") +
  ggsave("fito.png", height=6, width=9, dpi=600)
```
