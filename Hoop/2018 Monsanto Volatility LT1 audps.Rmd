...
```{r echo=TRUE}
Data <- read.csv("Monsanto Volatility Low Tunnel 1.csv", header=T)
str(Data)
...
names = c("Xtendimax", "Xtendimax + Durango DMA", "Xtendimax + RUP PMAX", "Xtendimax + RUP PMAX + Intact", "Xtendimax + RUP PMAX + Intact + MON 10",
            "MON 76981","MON 76981 + Liberty")
...
ggplot(data1, aes(x = Distance, y=Injury, group=Distance, fill=Injury)) +
  ...
  ggtitle("2018 Bayer Low Tunnel 1") +
  ...
  labs(y="Injury (%)", x="Distance from Plot Center (cm)") +
  ggsave("2018 Raw injury.png", units="in", width=10, height=8, dpi=600)
```

AUDPS analysis
```{r AUDPS/AUIDS calculations, echo=TRUE}
#changed plot number and re-filtered "data" and calculated audps using function below
data = Data %>%
  filter(DAT=="28", Plot=="308")
...
Data2 <- read.csv("Monsanto Volatility Low Tunnel 1 audps.csv", header=T)
str(Data2)
...
#failed normality test, testing transformations - sqrt is satisfactory transformation
Data2 <- Data2 %>%   
  mutate(tAUDPS = sqrt(AUDPS))
plot(density(Data2$tAUDPS))
qqnorm(Data2$tAUDPS); qqline(Data2$tAUDPS, col=2)
pearson.test(Data2$tAUDPS)
leveneTest(tAUDPS ~ Trtdes, data=Data2)

#model using sqrt transformation
model=lmer(sqrt(AUDPS) ~ Trtdes + (1|Rep), data=Data2)
summary(model)
anova(model, test.statistic = "F")

lsm <- emmeans(model, ~ Trtdes, type="response", adjust="none", contr="pairwise")
lsm
...
ggplot(cld, aes(x = Trtdes, y=response, label=.group, fill=response)) +
  ...
  geom_text(size = 3,
            ...
            nudge_y = c(190, 190, 190, 190, 190, 190, 190),
            color   = "black") + ylim(0, 850) +
  scale_y_continuous(breaks=c(0, 100, 200, 300, 400, 500, 600, 700, 800)) +
  scale_x_discrete(labels=wrap_format(30)) +
  ggsave("2018 AUDPS figure.tiff", units="in", width=10, height=8, dpi=600)
```

Weather figures
```{r Import data, echo=TRUE}
data=read.csv("Weather data Low Tunnel 1.csv")
str(data)
...
ggplot(data, aes(x = hours)) +
  ...
  labs(y="Air Temperature (°C)", x="Time following application (hours)", color="Parameter") +
  theme(legend.position=c(0.89,0.885)) +
  ggsave("2018 MV LT weather line.tiff", units="in", width=8, height=6, dpi=600)
```
...
plot +  #facet_wrap(~apprun) +  #labs(caption = "Oliveira et al. (2018) 96:1051-1063, the Plant Journal") +
    ggsave("2018 Wind Rose.png", units="in", width=6, height=5, dpi=600)
```
